...
const products = fetchedData?.data ?? [] as Product[];
  ...
  const showSkeleton = isFetching && debouncedQuery.length >= 2;
  const showEmpty =
    !showSkeleton &&
    !isError &&
    debouncedQuery.length >= 2 &&
    products.length === 0;
  const showResults = !showSkeleton && products.length > 0;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="min-h-80 bg-background p-6">
          {showIdle && <ProductSearchIdleState />}

          {showSkeleton && <ProductSearchSkeleton />}

          {isError && (
            <div className="rounded-2xl border border-destructive/20 bg-card p-6 text-right shadow-sm">
              <p className="text-sm font-semibold text-destructive">
                حدث خطأ أثناء تحميل النتائج.
              </p>
              <p className="mt-2 text-sm text-muted-foreground">
                تأكد من رابط API الخاص بالمنتجات داخل دالة{" "}
          ...
          {showEmpty && <ProductSearchEmptyState query={debouncedQuery} />}
              ...
              <div className="space-y-2">
                {products.map((product) => (
                  ...
                  >
                    ...
                    <ArrowUpLeft className="h-4 w-4 text-muted-foreground transition group-hover:text-primary" />
                  </Link>
                ))}
              </div>
            </div>
...
function ProductSearchIdleState() {
  return (
    <div className="flex h-70 flex-col items-center justify-center rounded-3xl border border-dashed border-border bg-card px-6 text-center shadow-sm">
      <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-teal/10 text-teal">
      ...
      {Array.from({ length: 5 }).map((_, index) => (
        ...
        >
          ...
          <div className="flex-1 space-y-2">
            <Skeleton className="h-4 w-2/3 rounded-full bg-muted" />
            <Skeleton className="h-3 w-1/2 rounded-full bg-accent" />
          </div>
        </div>
      ))}
    </div>
